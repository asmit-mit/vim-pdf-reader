...
Completions::Completions(const sf::Font &cmd_font, const sf::Font &desc_font, int font_size)
  ...
  for (std::size_t i = 0; i < max_list_items_; i++) {
    display_list_.emplace_back(sf::Text(cmd_font_, "", font_size));
    display_list_[i].setFillColor(fg_color_);
  }
}
...
void Completions::update() {
  ...
  if (list_dirty_) {
    ...
    for (std::size_t i = first_visible_; i < last; ++i, ++row) {
      display_list_[row].setString(completions_[i]);
      display_list_[row].setPosition(
          {utils::padding, display_area_.getPosition().y + row * utils::cmdline_height_}
      );

      if (i == selected_)
        display_list_[row].setFillColor(sf::Color::Yellow);
      else
        display_list_[row].setFillColor(fg_color_);
    }
...
void Completions::setTextColor(const sf::Color &color) {
  fg_color_ = color;
}
...
void Completions::setCompletionList(const std::vector<std::string> &list) {
  clear();
  completions_ = list;
